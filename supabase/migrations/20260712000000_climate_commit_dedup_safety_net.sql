-- Bug found via a real 98-file import: multiple staged files can resolve to
-- the same zone + metric + timestamp (or the same variety/phase + timestamp).
-- The caller (Node) is now responsible for collapsing same-batch duplicates
-- into exactly one canonical row per destination key BEFORE calling this
-- function — Postgres's `ON CONFLICT DO UPDATE` cannot affect the same
-- target row twice within one statement ("ON CONFLICT DO UPDATE command
-- cannot affect row a second time").
--
-- This migration adds a DISTINCT ON safety net to every INSERT ... SELECT in
-- commit_climate_import_batch, so this specific error can never recur here
-- even if a future bug ever lets duplicate keys reach this function again.
-- It keeps the LAST element per destination key (by array position) — this
-- is a defensive fallback only; correct identical-vs-conflict classification
-- and user-facing resolution happens in Node (climateDuplicates.ts), not here.
